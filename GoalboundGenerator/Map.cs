...
public class Map
{
    ...
    public DirectionNode GetPoint(int x, int y, Direction direction) => x < 0 || y < 0 || x >= width || y >= height ? invalidPoint : points[x][y][(int)direction];

    public void AddWall(int x, int y) => walls[x, y] = true;

    public bool IsWall(int x, int y) => x < 0 || y < 0 || x >= width || y >= height || walls[x, y];
